...
- [ ] 승인되지 않거나 격리된 콘텐츠는 피드에 포함되지 않는다.
- [ ] 키보드만으로 피드 항목을 탐색할 수 있다.
...
- [ ] 지역 또는 OTT 가용성이 확인되지 않은 링크는 활성화하지 않는다.
...
- [ ] 수집 실패가 기존의 마지막 정상 데이터를 삭제하지 않는다.
...
- [ ] 구조화된 조건은 사용자에게 확인 가능한 형태로 표시된다.
...
- [ ] 한국어와 영어 질의의 구조화 결과는 동일 스키마를 사용한다.
...
- [ ] 모든 핵심 기능은 키보드로 사용할 수 있다.
- [ ] 이미지 대체 텍스트, 포커스 표시, 색 대비, 스크린 리더 레이블을 제공한다.
...
- [ ] AI 계층은 하드 조건을 임의로 제거하거나 최종 순위를 확정하지 않는다.
...
- [ ] 최종 콘텐츠 ID는 모두 승인 카탈로그에 존재한다.
- [ ] 동일 입력과 동일 버전의 정책에서는 정의된 허용 범위 안에서 재현 가능한 결과를 제공한다.
...
- [ ] 스포일러를 최소화하고 AI 생성 문구임을 식별할 수 있다.
- [ ] AI가 생성한 새 작품·제공처·줄거리 사실을 카탈로그에 추가하지 않는다.
...
- [ ] 후속 요청도 US-010과 US-011의 적격성·근거 검증을 동일하게 통과한다.
...
- [ ] 사용자 확인 전에는 모호한 값을 하드 조건으로 확정하지 않는다.
...
- [ ] 콜드 스타트에서는 명시적 선호와 인기·신작 신호를 결합한다.
...
- [ ] 오래되거나 격리된 메타데이터는 알림을 발생시키지 않는다.
...
- [ ] 개인정보와 모델 내부 추론 내용은 추적 로그에 기록하지 않는다.
- [ ] 추적 로그 접근은 운영자 권한으로 제한된다.
...
- [ ] 구조화 로그, 요청 상관관계 ID, 지연·오류·처리량 지표를 제공한다.
- [ ] 장애 대응은 탐지, 영향 확인, 완화, 복구, 공지, 사후 분석을 포함한다.
...
- [ ] 핵심 회원 흐름은 키보드와 보조기술로 완료할 수 있다.
- [ ] 상용 전환 전 개인정보·접근성 법무 및 전문가 검토가 완료되어야 한다.
