...
## U04 Operational Boundary

- Provider-specific production endpoints and credentials are deployment inputs and are intentionally absent from the repository.
- `worker-ingestion` has the only non-internal provider egress network. Production network policy must further restrict that network to approved provider origins.
- GitHub Actions remain manual-only until every paused workflow passes a controlled run; local and PostgreSQL quality gates are the current release evidence.
